...
// get paper list data
router.route('/getPaperTypeList').get(async function (request, response) {
  try {
    var query ='SELECT * FROM tbl_paper_type';
       [rows, fields]  =  await connection.query(query);
      let list = Object.values(JSON.parse(JSON.stringify(rows)));
      response.send(list)
    } catch (error) {
  ...
  router.route('/getPaperNameList/:id').get(async function (request, response) {
    try {
      var query =`SELECT paper_list_id as id ,name as name  FROM tbl_paper_list where paper_id = ${request.params.id}`;
         [rows, fields]  =  await connection.query(query);
        let list = Object.values(JSON.parse(JSON.stringify(rows)));
        response.send(list)
      } catch (error) {
   ...
   // get topic list 
   router.route('/getMainsTopicList/:id/:paper_id').get(async function (request, response) {
    try {
      var query =`SELECT id as id,subject as name FROM tbl_mains_subject_list where paper_id =${request.params.id} and paper_list_id = ${request.params.paper_id}`;
         [rows, fields]  =  await connection.query(query);
        let list = Object.values(JSON.parse(JSON.stringify(rows)));
        response.send(list)
      } catch (error) {
    ...
    router.route('/getQuizMainsQuestion/:id').get(async function (request, response) {
      try {
        var query =`SELECT * FROM tbl_mains_question as r where r.topic_id =${request.params.id}`;
            [rows, fields]  =  await connection.query(query);
            let list = Object.values(JSON.parse(JSON.stringify(rows)));
            response.send(list)
        } catch (error) {
