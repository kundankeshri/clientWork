...
exports.getQuizMainsQuestion = async(req, res) =>{
  try {
    console.log('kundan')
    let id = 1;
    let topicId =req.params.id;
    let questions = [];
    let query = `SELECT * FROM tbl_mains_question where topic_id = ${id} `;
    
       [rows, fields]  =  await connection.query(query);
   
    let options = Object.values(JSON.parse(JSON.stringify(rows)));
    let query1 = `SELECT * FROM   tbl_mains_subject_list as t 
    where  t.id = ${topicId}  `;
    try {
       [rows1, fields1]  =  await connection.query(query1);
    } catch (error) {
    ...
    let topicType = Object.values(JSON.parse(JSON.stringify(rows1)));
  
    if(options && options.length > 0){
      let distinctQuestionList = options.map(item => item.topicId)
      .filter((value, index, self) => self.indexOf(value) === index);
      for(let i=0;i< distinctQuestionList.length;i++){
         let obj ={
             'id': options[distinctQuestionList[i]],
